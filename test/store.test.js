...
test('addManagedDevice validates the MAC and the IP before storing anything', async () => {
  const gladys = createFakeGladys();
  await assert.rejects(addManagedDevice(gladys, { name: 'x', mac: 'nope', ip: '192.168.1.1' }));
...
test('addManagedDevice validates an explicit port', async () => {
  const gladys = createFakeGladys();
  await assert.rejects(
    addManagedDevice(gladys, {
      name: 'x',
...
test('removeManagedDevice removes the matching device and reports whether it existed', async () => {
  const gladys = createFakeGladys();
  const device = await addManagedDevice(gladys, {
    name: 'PC',
    mac: 'aa:bb:cc:dd:ee:ff',
    ip: '192.168.1.10',
  });
